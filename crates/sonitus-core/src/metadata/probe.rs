...
pub fn extract_tags(bytes: &[u8]) -> Result<ParsedTags> {
    ...
    if let Some(track) = format.default_track() {
        ...
        if let Some(rate) = cp.sample_rate { out.sample_rate_hz = Some(rate as i32); }
        if let Some(bits) = cp.bits_per_sample { out.bit_depth = Some(bits as i32); }
        if let Some(ch) = cp.channels.map(|c| c.count()) { out.channels = Some(ch as i32); }
...
fn apply_tag(out: &mut ParsedTags, tag: &symphonia::core::meta::Tag) {
    ...
    if let Some(k) = tag.std_key {
        match k {
            ...
            K::Date => out.year = ParsedTags::parse_year(&value_str),
            K::TrackNumber => {
                let (n, t) = ParsedTags::parse_track_number(&value_str);
                out.track_number = n;
...
impl ParseHelpers for ParsedTags {
    fn parse_year(s: &str) -> Option<i32> {
